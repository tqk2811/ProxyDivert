...
public static class OutboundSignature
{
    ...
    public static string Of(Outbound outbound, string? wireProxyPath = null)
    {
        ...
        // A VPN's real settings live in the .conf file the outbound merely points at, so the path
        // alone would not notice the user editing it. Stamping the file makes "I changed my
        // WireGuard keys" reconnect the tunnel, which is what the user expects to happen.
        if (outbound.Kind == OutboundKind.Vpn)
        {
            sb.Append('|').Append(wireProxyPath).Append('|').Append(StampFile(outbound.Url));
        }
